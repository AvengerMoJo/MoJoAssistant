    ...
    def load_config(self, config_file: str) -> None:
        ...
        try:
            ...
            if (
                ...
                # If default interface doesn't exist but we have interfaces, set the first one
                first_interface = next(iter(self.interfaces.keys()))
                print(
                    f"⚠️  Default interface '{config.get('default_interface')}' not found"
                )
                print(f"  Setting active interface to: {first_interface}")
...
def create_llm_interface(
        ...
        default_model_paths = {
            "default": {
                "path": os.path.expanduser(
                    "~/AppData/Local/nomic.ai/GPT4All/ggml-model-gpt4all-falcon-q4_0.bin"
                ),
                "type": "gptj",
            },
            "llama": {
                "path": os.path.expanduser(
                    "~/AppData/Local/nomic.ai/GPT4All/mistral-7b-instruct-v0.1.Q4_0.gguf"
                ),
                "type": "llama",
            },
        }
    ...
    if os.path.exists(model_path):
        ...
        print(
            "WARNING: No local LLM model found. Please configure a model path or API endpoint."
        )
