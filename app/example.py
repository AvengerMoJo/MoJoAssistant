...
def example_usage():
    ...
    """
    Example of how to use the MemoryService
    """
    ...
    memory_service.add_user_message(user_query)
    memory_service.add_assistant_message(assistant_response)
    
    # User interaction 2
    user_query = "I'm trying to understand how to use decorators in Python"
    
    # Get context for this query
    context = memory_service.get_context_for_query(user_query)
    ...
    memory_service.update_memory_from_response(user_query, assistant_response)
    ...
    """
    memory_service.add_to_knowledge_base(python_doc, {"topic": "Python", "subtopic": "Decorators"})
    
    # End the conversation and store it
    memory_service.end_conversation()
    
    # Save memory state
    memory_service.save_memory_state("memory_state.json")
