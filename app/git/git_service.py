...
class GitService:
    ...
    def add_repository(self, repo_name: str, repo_url: str, ssh_key_path: str,
    ...
    def get_file_content(self, repo_name: str, file_path: str,
                        git_hash: Optional[str] = None, update_repo: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if update_repo:
                ...
                if update_result["status"] != "success":
                    ...
                    logger.warning(f"Failed to update repository {repo_name}: {update_result['message']}")
            ...
            if git_hash:
                try:
                    ...
                    return {"status": "error", "message": f"File {file_path} not found at hash {git_hash}"}
            else:
    ...
    def remove_repository(self, repo_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if repo_path.exists():
                ...
                return {"status": "success", "message": f"Repository {repo_name} removed"}
            else:
                return {"status": "error", "message": f"Repository {repo_name} not found"}

        except Exception as e:
            logger.error(f"Failed to remove repository {repo_name}: {e}")
            return {"status": "error", "message": f"Failed to remove repository: {str(e)}"}
    ...
    def _update_repository(self, repo_name: str) -> Dict[str, Any]:
        ...
        # Configure SSH
        ssh_command = f"ssh -i {repo_config['ssh_key_path']} -o StrictHostKeyChecking=no"
