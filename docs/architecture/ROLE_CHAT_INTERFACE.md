...
When Researcher finishes a research task, the result lands in the HITL inbox as a
...
- Researcher receiving work from two directions: scheduler + user chat
...
- The user unable to tell what state Researcher is in
...
When the user opens a chat session with Researcher:

1. **Her personality loads** — the system prompt from `~/.memory/roles/researcher.json`
   is active. She responds as Researcher, not as a generic assistant.
...
3. **She answers from accumulated knowledge** — Researcher can explain her Trivy
   findings, connect ideas across multiple research sessions, clarify her
   reasoning — all from `~/.memory/roles/researcher/knowledge_units/` and her
...
- **OpenAI-compatible proxy** — `/v1/models` returns available roles; `/v1/chat/completions`
  ...
  can talk to Researcher directly using the OpenAI API format.
...
- **Cross-role referral** — Researcher can say "Analyst would know more about this"
  and the client can open a handoff chat to Analyst with context carried over.
...
2. **State conflict** — if Researcher is already running a background task,
